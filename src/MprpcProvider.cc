...
json   protobuf
*/
// 这里是框架提供给外部使用的，可以发布rpc方法的函数接口（注册服务对象和其对应的PRC方法，以便服务端处理客户端的请求）
/*
    //service服务类型信息
    struct ServiceInfo
    {
...
void MprpcProvider::SendRpcResponse(const muduo::net::TcpConnectionPtr& conn, google::protobuf::Message *response)
{
    ...
    if (!google::protobuf::util::MessageToJsonString(*response, &response_str, json_options).ok())
    {
        std::cout << "JSON response serialization error!" << std::endl;
        return;
    }
#else
    if (!response->SerializeToString(&response_str))
    {
        std::cout << "Protobuf response serialization error!" << std::endl;
        return;
    }
#endif
